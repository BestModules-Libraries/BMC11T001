...
History:         
V1.0.1   -- initial version；2023-03-16；Arduino IDE : v1.8.16
******************************************************************/
#include "BMC11T001.h"
/**********************************************************
...
/**********************************************************
Description: Read BC45B4523 single regiseter value
Parameters:  Sector : Sector0/Sector1 
             addr : 
                   Sector 0 ：00h~08h，09h~0Fh，11h~17h，19h~27h，29h~37h，39h，3Ah，3Ch，3Eh，3Fh
                   Sector 1 ：00h~08h，00h~05h，08h，09h，0Bh，0Dh~13h，2Eh，2Fh，37h
             buff[]: Used to store BC45B4523 single regiseter value  
Return:      Length : The length of the board reply string                       
Others:            
**********************************************************/
uint8_t BMC11T001::readRegister_ADVANCED(uint8_t sector,uint8_t addr,uint8_t buff[])
{
  int Length;
  uint8_t Read_String[] = "reg -rd -s0 00\r\n";
  uint8_t Data[1]; 
  Data[0] = addr;
  if(sector == SECTOR0)
  {
  ...
  byteToChar(&Data[0],&Read_String[12],&Read_String[13]);
  writeBySerial(Read_String, sizeof(Read_String));
  delay(10);
  Length = getEvent(Advanced_Read_Register,buff);
  return Length;
}

/**********************************************************
Description: Read BC45B4523 all Register Value
Parameters:  buff[]: Used to store BC45B4523 single regiseter value 
Return:      Length : The length of the board reply string                            
Others:            
**********************************************************/

uint16_t BMC11T001::readAllRegister_ADVANCED(uint8_t buff[])
{
  uint16_t Length = 0;
  uint8_t Read_String[] = "reg -rd -all\r\n";
  writeBySerial(Read_String, sizeof(Read_String));
  while(Length != 2164)
  {
   Length = getEvent(Advanced_Read_All_Register,buff); 
  }
  return Length;
}

/**********************************************************
Description: Write BC45B4523 Register Value
Parameters:  Sector : Sector0/Sector1 
             addr : 
                   Sector 0 ：00h~08h，09h~0Fh，11h~17h，19h~27h，29h~37h，39h，3Ah，3Ch，3Eh，3Fh
                   Sector 1 ：00h~08h，00h~05h，08h，09h，0Bh，0Dh~13h，2Eh，2Fh，37h
             value: Data written to EEPROM(1 byte)  
Return:      Length : The length of the board reply string                       
Others:            
**********************************************************/

void BMC11T001::writeRegister_ADVANCED(uint8_t sector,uint8_t addr,uint8_t value)
{
  uint8_t buff[40];
  uint8_t Write_String[] = "reg -wr -s0 00 00\r\n";
  uint8_t Data[2]; 
  Data[0] = addr;
  Data[1] = value;
  if(sector == SECTOR0)
  ...
  byteToChar(&Data[0],&Write_String[12],&Write_String[13]);
  byteToChar(&Data[1],&Write_String[15],&Write_String[16]);
  writeBySerial(Write_String, sizeof(Write_String));
  delay(5);
  getEvent(Advanced_Write_Register,buff);
}

/**********************************************************
Description: Write all Register DefaultValue
Parameters:  
Return:                           
Others:            
**********************************************************/
void BMC11T001::writeDefaultValueRegister_ADVANCED(void)
{
  uint8_t buff[40];
  uint8_t Write_String[] = "reg -wr -default\r\n";
  writeBySerial(Write_String, sizeof(Write_String));
  delay(10);
  getEvent(Advanced_Write_DefaultValue_Register,buff);
}
